...
interface DisplayLeg { flights: DisplayFlight[] }
interface DisplayScenario { legs: DisplayLeg[]; totalPrice: number }
...
export default function Results() {
  ...
  useEffect(() => {
    ...
    setScenarios(transformScenarios(JSON.parse(stored)))
  }, [navigate])
                      ...
                      <h3 className="font-semibold text-lg mb-3">
                        ...
                        {leg.flights[0] && (
                          <span className="text-sm font-normal text-gray-500 ml-2">
                            {leg.flights[0].departure} → {leg.flights[0].arrival} · {leg.flights[0].departureDate}
                          </span>
                        )}
                      </h3>
                      {leg.flights.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {leg.flights.map((flight, fi) => {
                            ...
                            return (
                              <div key={fi} className={`p-4 rounded-lg border-2 transition-all ${isCheapest ? 'border-green-500 bg-green-50' : 'border-gray-200 bg-white'}`}>
                                <div className="flex items-center justify-between">
                                  <div className="flex-1">
                                    <div className="font-semibold text-blue-600 mb-1">{flight.airline || 'Unknown airline'}</div>
                                ...
                                {isCheapest && <Badge className="mt-2 bg-green-600 text-white">Cheapest</Badge>}
                              </div>
                            )
                          })}
                        </div>
